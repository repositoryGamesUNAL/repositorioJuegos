      ...
      {steps.map((step: StepItem, index: number) => (
        ...
        >
          <div className={`${styles[`content--${direction}`]}`}>
            <div
              className={`${styles.circle} ${styles[`circle--${step.status}`]}`}
            >
            ...
            <p className={`${styles.label} ${styles[`label--${step.status}`]}`}>
              {step.description}
            </p>
          </div>
          {index < steps.length - 1 && ( // Añadir la barra solo si no es el último paso
            <div
              className={`${styles[`bar--${direction}`]} ${
                step.status === "error" || step.status === "waiting"
                  ? styles["bar--disabled"]
                  : ""
              }`}
            ></div>
